...
def home(request):
    return render(request, 'demand/calculate.html')


def get_demand_page(request):
    return render(request, 'demand/calculate.html')
...
def get_combined_population(request):
    ...
    try:
        ...
        if intermediate_stage:
            if intermediate_stage.strip() == "":  # Handle empty input
                return JsonResponse({"intermediate_stage_population": 0}, safe=False)
            elif intermediate_stage.isdigit() and 1 <= int(intermediate_stage) <= 50:
